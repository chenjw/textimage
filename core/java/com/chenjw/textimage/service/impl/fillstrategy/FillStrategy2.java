...
/**
 * 填充算法2 创建队列来管理空闲块，目前用的是這個算法，主要逻辑:<br>
 * 1. 查询“空闲块链表”。<br>
 ...
 */
public class FillStrategy2 implements FillStrategy {
	...
	public List<Dimension> tryFillCanvas(TextImageContext context) {
		...
		// 填充
		return _fill(textLineListMap, maxLineHeight, maxWidth, maxCanvasHeight);
	}
	 ...
	 * @return
	 */
	private int _calculateMaxWidth(TextInfo textInfo, StyleConfig styleConfig) {
		// 1.遍历所有行，计算出现次数最多的行最大宽度值
		Map<Integer, Integer> countMap = new HashMap<Integer, Integer>();
		for (Entry<String, TextField> textFieldentry : textInfo
				.getTextFieldMap().entrySet()) {
			String key = textFieldentry.getKey();
			int lineWidth = styleConfig.getTextStyle(key).getLineWidth();
	 ...
	 * @return
	 */
	private List<Dimension> _fill(Map<Integer, List<TextLine>> textLineListMap,
			int maxLineHeight, int maxWidth, int maxHeight) {
		int height = maxLineHeight;
		// 当前位置
		int nY = 0;
		// 用来放空闲块的链表
				...
				for (TextLine textLine : list) {
					...
					if (p == null) {
						if (nY + textLine.getHeight() > maxHeight) {
							result.add(new Dimension(maxWidth, nY));
							nY = 0;
						}
						textLine.setX(0);
						textLine.setY(nY);
						textLine.setImageIndex(result.size());
						if (textLine.getWidth() < maxWidth) {
							freeBlockList.addBlock(textLine.getWidth(), nY,
									maxWidth - textLine.getWidth(),
									textLine.getHeight(), result.size());
						}
